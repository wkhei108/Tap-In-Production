...
import { resolveProjectBySlug, resolveRelatedProjects } from "@/lib/campaigns";
...
export default async function ProjectPage({
  ...
  const breadcrumbs = breadcrumbJsonLd([
    ...
    { name: dict.nav.work, path: pathFor(locale, "work") },
    {
      ...
      path: pathFor(locale, "work", project.slug),
    },
  ]);
    ...
    <>
      ...
      <section className="relative isolate overflow-hidden pt-28 md:pt-36">
        ...
        <div className="shell">
          ...
          <DisplayText
            as="h1"
            locale={locale}
            className="mt-5 max-w-[20ch] text-display-xl"
          >
      ...
      <section className="shell py-14 md:py-20">
        <dl className="grid gap-8 border-y border-line py-8 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {project.services[locale].length > 0 ? (
            <div className="sm:col-span-2 lg:col-span-1">
              <dt className="meta text-mute/70">
                {dict.common.servicesDelivered}
              </dt>
              <dd className="mt-2">
        ...
        {/* 4–6. Brief, challenge, approach.
             Campaigns created in the admin tool carry a short record and have
             none of this prose, so each section renders only when it exists —
             a shorter page rather than a page of empty headings. */}
        {project.brief || project.challenge || project.approach ? (
          <div className="mt-16 grid gap-10 lg:grid-cols-12 lg:gap-14">
            {project.brief ? (
              <div className="lg:col-span-4">
                <h2 className="meta text-lime">{dict.common.brief}</h2>
                <p className="mt-4 text-lead text-bone/90">
      ...
      <section className="shell py-14 md:py-20">
        <div className="grid gap-10 lg:grid-cols-12 lg:gap-14">
          {project.deliverables[locale].length > 0 ? (
              ...
              <ul className="mt-6 flex flex-col">
                {project.deliverables[locale].map((item) => (
                  ...
                  >
                    <span
                      aria-hidden="true"
                      className="mt-1.5 size-1 shrink-0 rotate-45 bg-lime"
                    />
                    {item}
                  </li>
                ))}
